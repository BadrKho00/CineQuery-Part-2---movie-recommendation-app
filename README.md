...
│   ├── app.py                # Routes: /autocomplete, /recommend, /index-movies
│   ├── bq_client.py          # BigQuery queries and ML.RECOMMEND calls
│   ├── es_client.py          # Elasticsearch indexing and autocomplete
│   ├── tmdb_client.py        # TMDB poster fetching
│   ├── title_utils.py        # Title formatting (e.g. "Matrix, The" → "The Matrix")
│   ├── config.py             # Environment variables and constants
│   ├── requirements.txt
│   └── Dockerfile
├── frontend/                 # Streamlit UI
│   ├── app.py                # Search, selection, recommendations display
│   ├── requirements.txt
│   └── Dockerfile
├── datasets/                 # MovieLens ml-small CSVs (uploaded to BigQuery)
│   ├── ml-small-movies.csv
│   ├── ml-small-ratings.csv
│   └── ml-small-links.csv
├── train_model.sql           # BigQuery ML model training script
├── docker-compose.yml        # Local development (both containers)
└── README.md
```
...
### Prerequisites
- Docker Desktop
- A `gcp-key.json` service account key (BigQuery access) placed in the project root — not included in the repo for security reasons
...
Then index movies into Elasticsearch (run once after startup):
